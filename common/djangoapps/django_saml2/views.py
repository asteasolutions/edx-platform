...
from django_saml2.cache import IdentityCache, OutstandingQueriesCache
from django_saml2.cache import StateCache
from django_saml2.conf import get_config
from django_saml2.signals import post_authenticated
from django_saml2.utils import get_custom_setting, available_idps, get_location


logger = logging.getLogger('django_saml2')
...
def login(request,
          ...
          wayf_template='django_saml2/wayf.html',
          authorization_error_template='django_saml2/auth_error.html'):
...
from student.models import (Registration, UserProfile, PasswordHistory)
...
def create_user(request,
                external_id=None,
                external_email=None):
    logger.error('create_user started')

    external_id = '1427207157'
    external_domain = 'http://wqu.edvera.dev:3000/'

    external_name = 'JohnRambo2'
    external_email = 'user705@user.com'

    user_query_args = {'username': external_name,
    ...
    if created:
        ...
        UserPreference.set_preference(user, LANGUAGE_KEY, 'EN')
...
def assertion_consumer_service(request,
    ...
    django_saml2.backends.Saml2Backend that should be
...
def echo_attributes(request,
                    config_loader_path=None,
                    template='django_saml2/echo_attributes.html'):
...
def do_logout_service(request, data, binding, config_loader_path=None, next_page=None,
                   logout_error_template='django_saml2/logout_error.html'):
